...
* БД - подход Code First, переключение провайдера в зависимости от конфигурации сервиса (либо в памяти, либо по MSSQL-connectionString) - пока без разделения R/W
* Разделение read/write операций - CQRS (пока без генерации событий)
* Слой бизнес логики (BLL) использует механизм построения логической цепочки (Blc) для определения взаимосвязи между логическими шагами (команды/запросы вызываются внутри шага)<br>
IBusinessProcess -> IBusinessProcessStep -> ICommand/IQuery
* Использование сервисов DI внутри класса, через инициализацию свойств класса, вместо инициализации параметров конструктора (см. ServiceProviderPropertyCreator)
...
* *setup-boot2docker.bat* - монтирование диска C к виртуальной docker-машине (см.комментарии в скрипте)
